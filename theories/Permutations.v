...
Proof.
  ...
  do 2 rewrite length_app. rewrite IHHP1. rewrite IHHP2.
  reflexivity.
Qed.  
...
Definition bind {A B} (o : option A) (k : A -> option B) :=
  match o with
  | Some a => k a
  | None => None
  end.
...
Proof.
  ...
  - simpl in H.
    ...
    apply IHb2 in HEQ2.
    subst. rewrite length_app. reflexivity.
    inversion H.
    inversion H.
    inversion H.
Qed.    
...
Lemma length_app_inversion : forall (A:Type) (xs1 xs2 ys1 ys2 : list A),
...
Proof.
  ...
  - rewrite Nat.eqb_refl. reflexivity.
  - unfold bind. rewrite IHp1. rewrite <- bij_list_coerce. assumption.
  - unfold perm_bij_clause_4, perm_bij_clause_4_clause_1.
    ...
    { rewrite length_app. assert (length ?(l11) = length l11) by reflexivity. rewrite H. lia. }
    rewrite H.
    apply split_option_correct in H.
    destruct H as [HEq1 [HEq2 HEq3]].
    destruct (length_app_inversion _ l11 l12 l11' l12' HEq1). apply symmetry. apply HEq2.
